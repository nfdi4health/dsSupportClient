...
#' @details For each connected study, the function retrieves the column names of the server-side data.frame via dsBaseClient::ds.colnames, then calls the supplied ds_function once per variable, building one data.frame of results per study with a temporary "rn" row-name column. These per-study tables are then combined by that "rn" column using dplyr::full_join inside purrr::reduce, after which "rn" is restored as the row names. There is no single fixed server function called: ds.wrapper dispatches whichever DataSHIELD client aggregate function is passed via ds_function, and that function issues its own server-side call(s).
